...
const lights = grid(1000, 0);
const operations = {
    'on': (x: number, y: number) => lights[x][y] + 1,
    'off': (x: number, y: number) => Math.max(lights[x][y] - 1, 0),
    'toggle': (x: number, y: number) => lights[x][y] + 2
}
...
const litCount = lights.reduce((acc, val) => acc + val.reduce((a, v) => a + v, 0), 0);
